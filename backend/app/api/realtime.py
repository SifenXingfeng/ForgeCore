...
from app.services.agent_run_service import load_agent_run
...
async def stream_agent_events(session_id: str, user: CurrentUser, db: DbSession) -> EventSourceResponse:
    session = await get_agent_session(session_id)
    run = None if session is not None else await load_agent_run(session_id, db)
    if session is None and run is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Agent 会话或 run 不存在")
