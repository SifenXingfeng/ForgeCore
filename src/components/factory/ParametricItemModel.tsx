...
import {
  ...
  buildRuntimeItemModel,
  type RuntimeBuildResult,
  type RuntimeMaterialPreset,
...
function createProceduralTexture(bindingKey: string): THREE.CanvasTexture | null {
  ...
  context.strokeStyle = `hsl(${Math.abs(hash) % 360} 20% 72%)`
  context.lineWidth = 4
...
function materialFromPreset(
  preset: RuntimeMaterialPreset | undefined,
  isPrimary: boolean,
  ...
  const material = new THREE.MeshStandardMaterial({
    color: colorOverride ?? new THREE.Color(sourceColor[0], sourceColor[1], sourceColor[2]),
    emissive: emission ?? new THREE.Color(0, 0, 0),
    emissiveIntensity: emission ? 0.72 : 0,
    ...
    opacity,
    transparent: opacity < 0.999 || source.alphaMode === 'BLEND',
...
function createCachedModel(build: RuntimeBuildResult, overrides: ModelParameters): CachedModel {
  const requestedPreset = build.parameters.materialPreset
  const primaryMaterialId = typeof requestedPreset === 'string' && requestedPreset !== 'auto'
    ? requestedPreset
    : Object.values(build.definition.defaultMaterials)[0] ?? 'neutral'
  const primitives = build.geometry.primitives.map((primitive) => {
    ...
    geometry.setAttribute('normal', new THREE.Float32BufferAttribute(primitive.normals, 3))
    geometry.setAttribute('uv', new THREE.Float32BufferAttribute(primitive.uvs, 2))
    geometry.setIndex(primitive.indices)
    geometry.computeBoundingBox()
    geometry.computeBoundingSphere()
    const material = materialFromPreset(RUNTIME_MATERIAL_LIBRARY[primitive.material], primitive.material === primaryMaterialId, overrides)
    return { geometry, material }
  })
  return { primitives, bounds: build.geometry.bounds }
}

function useParametricModel(modelId: string, parameters: ModelParameters): CachedModel | null {
  const parameterKey = stableParameterKey(parameters)
  return useMemo(() => {
    const cacheKey = `${modelId}:${parameterKey}`
    const cached = modelCache.get(cacheKey)
    if (cached) return cached
    try {
      const build = buildRuntimeItemModel(modelId, parameters)
      const model = createCachedModel(build, parameters)
      modelCache.set(cacheKey, model)
      return model
    } catch (error) {
      console.warn('ForgeCore parametric item model fallback activated', error)
      return null
    }
  }, [modelId, parameterKey, parameters])
}

export interface ParametricItemModelProps {
  modelId: string
  parameters: ModelParameters
  targetSize?: [number, number, number]
  referenceTargetSize?: [number, number, number]
  center?: boolean
  fallback?: ReactNode
}

export function ParametricItemModel({ modelId, parameters, targetSize, referenceTargetSize, center = false, fallback = null }: ParametricItemModelProps) {
  const model = useParametricModel(modelId, parameters)
  const referenceModel = useParametricModel(modelId, EMPTY_PARAMETERS)
  if (!model) return fallback
  const size = model.bounds.size
  const fitSize = referenceTargetSize && referenceModel ? referenceModel.bounds.size : size
  const fitTarget = referenceTargetSize ?? targetSize
  const scale = fitTarget
    ? Math.min(
        fitTarget[0] / Math.max(fitSize[0], 0.0001),
        fitTarget[1] / Math.max(fitSize[1], 0.0001),
        fitTarget[2] / Math.max(fitSize[2], 0.0001),
      )
    : 1
  const centerY = center ? -((model.bounds.min[1] + model.bounds.max[1]) / 2) * scale : 0
  return (
    <group scale={scale} position={[0, centerY, 0]}>
      {model.primitives.map((primitive, index) => (
        <mesh key={index} geometry={primitive.geometry} material={primitive.material} castShadow receiveShadow />
      ))}
    </group>
  )
}
...
export function ParametricModelPreview({ modelId, parameters, label }: { modelId: string; parameters: ModelParameters; label: string }) {
  return (
    <div className="parametric-model-preview" aria-label={`${label} 参数化三维实时预览`}>
      <Canvas orthographic camera={{ position: [4, 3.2, 4], zoom: 64, near: 0.1, far: 100 }} dpr={[1, 1.5]} shadows>
        <color attach="background" args={['#edf0eb']} />
        <ambientLight intensity={1.45} />
        <directionalLight position={[4, 7, 5]} intensity={2.2} color="#fff8dc" castShadow />
        <directionalLight position={[-4, 2, -3]} intensity={0.75} color="#c7d2db" />
        <group rotation={[0.08, -0.55, 0]}>
          <ParametricItemModel modelId={modelId} parameters={parameters} targetSize={[2.5, 2.5, 2.5]} center />
        </group>
        <OrbitControls makeDefault enablePan={false} enableZoom minZoom={42} maxZoom={105} autoRotate autoRotateSpeed={0.7} />
      </Canvas>
      <span>实时几何 · 可拖动查看 · 滚轮缩放</span>
    </div>
  )
}
